...
public class BowlerFile {
	...
	public static Bowler getBowlerInfo(String nickName)
		...
		try(BufferedReader in = new BufferedReader(new FileReader(Constants.BOWLER_DAT));)
                {
		String data;
		while ((data = in.readLine()) != null) {
			...
			if (nickName.equals(bowler[0])) {
                            logger.log(Level.INFO, "Nick: {0} Full: {1} email: {2}.", new Object[] {bowler[0], bowler[1], bowler[2]});
				return (new Bowler(bowler[0], bowler[1], bowler[2]));
			}
		}
                logger.log(Level.INFO,"Nick not found in the database...");
		return null;
                }
	}
	...
	public static void putBowlerInfo(
		...
		try(RandomAccessFile out = new RandomAccessFile(Constants.BOWLER_DAT, "rw");)
                {
	...
	public static List <String> getBowlers()
		throws IOException {

		LinkedList <String> allBowlers = new LinkedList<>();
