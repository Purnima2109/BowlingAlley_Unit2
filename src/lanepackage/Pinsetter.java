...
public class Pinsetter {
    ...
    SecureRandom rnd;
    private ArrayList<PinsetterObserver> subscribers;
     ...
     * incremented
     */
    public void ballThrown(int angle, int velocity) {	// simulated event of ball hits sensor
//        System.out.println("Angle"+angle+"velo"+velocity);
        int count = 0;
        foul = false;
        double skill = rnd.nextDouble();
        if (angle <= 30 || velocity <= 5) {
            count = 0;
        } 
        else  {
            for (int i = 0; i <= 9; i++) {
